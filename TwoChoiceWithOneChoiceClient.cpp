...
TwoChoiceWithOneChoiceClient::TwoChoiceWithOneChoiceClient(long numOfDataSets, bool inMemory, bool overwrite, bool profile) {
    ...
    oneChoiceServer = new OneChoiceServer(numOfDataSets, inMemory, overwrite, profile, false);
    ...
    for (auto pair : sorted) {
        ...
        if (pss > mpl) {
            ...
            *(long*) (&(cntstr[AES_KEY_SIZE - 5])) = -1;
            ...
            //            printf("one-choice: index:%d keyword:%s pos:%d length:%d\n", index, pair.first.c_str(), pos, pair.second.size(), pair.second.size());
            ...
            *(long*) (&(cntstr[AES_KEY_SIZE - 5])) = -1;
            ...
            //            printf("two-choice: index:%d keyword:%s cipherindex:%d length:%d superbin:%d pos1:%d pos2:%d newsize:%d\n", index, pair.first.c_str(), cipherIndex, pair.second.size(),superBins,pos1,pos2,newsize);
...
void TwoChoiceWithOneChoiceClient::destry(long index) {
